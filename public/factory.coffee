...
#        news.dic[_news.id] = _news
  ...
  policies = {
    ...
    refresh: ()->
      $http.get("/policies")
      .then (res)->
        policies.data.splice(0, policies.data.length)
        for _policies,i in res.data
          _policies.index = i
          policies.data.push(_policies)
          policies.dic[_policies.id] = _policies
      .catch (err)->
        console.log err
        alert(err.data.message)
    push: (_policies)->
      $http.post("/policies", _policies)
      .then (res)->
...
#        policies.dic[_policies.id] = _policies
    ...
    update: (_policies)->
      $http.post("/policies/#{_policies.id}", _policies)
      .then (res)->
#        return if not policies.dic[_policies.id]?.index
#        _policies.index =policies.dic[_policies.id].index
#        policies.data[_policies.index] = _policies
#        policies.dic[_policies.id] = _policies
        alert("Updated successfully")
        policies.refresh()
        return res.data
    ...
    delete: (_policies)->
      $http.delete("/policies/#{_policies.id}")
      .then (res)->
        alert("Deleted successfully")
        policies.refresh()
  }
